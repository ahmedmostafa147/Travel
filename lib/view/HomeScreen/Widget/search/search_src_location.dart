...
class SearchSrcLocationField extends StatefulWidget {
  const SearchSrcLocationField({
    Key? key,
  }) : super(key: key);

  @override
  State<SearchSrcLocationField> createState() => _SearchSrcLocationFieldState();
}

class _SearchSrcLocationFieldState extends State<SearchSrcLocationField> {
  ...
  void applyLocation(String from) async {
              ...
              child: TextField(
                ...
                onChanged: (value) {
                  if (value.isNotEmpty) {
                    isSearchActive = true;
                    setState(() {
                      searchResult = documents.where((from) {
                        return from.toLowerCase().contains(value.toLowerCase());
                      }).toList();
                    });
                  } else {
                    setState(() {
...
List removeRepeated(documents) {
  Set uniqueData = {};
  documents.forEach((way) => uniqueData.add(way['from']));
